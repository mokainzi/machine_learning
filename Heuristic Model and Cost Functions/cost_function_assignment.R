...
data=sample(2,nrow(heuristic_df), replace =TRUE, prob=c(0.70,0.30))
h_train= heuristic_df[data==1,]
h_test= heuristic_df[data==2,]
nrow(h_train)
nrow(h_test)
...
accuracy <- (sum(heuristic_df$True.Positive) + sum(heuristic_df$True.Negative)) / length(heuristic_df$Guess)
accuracy
